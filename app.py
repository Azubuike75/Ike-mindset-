...
def highest_chapter_number(manuscript_text: str) -> int:
    """Scans the manuscript for 'Chapter <N>' or 'Chapter <One/Two/...>' headings
    and returns the highest number found (0 if none). This is what the AI's next
    chapter number is based on — not the model's own guess, which is unreliable
    over long manuscripts and previously caused it to restart numbering mid-book."""
    import re
    highest = 0
    for match in re.finditer(r"(?im)^chapter\s+([a-z]+|\d+)\b", manuscript_text):
        token = match.group(1).lower()
        if token.isdigit():
            n = int(token)
        else:
            n = _CHAPTER_WORD_NUMBERS.get(token, 0)
...
def generate_next_chapter(state: dict) -> str:
    # Backward-compat: projects created before chapter-number tracking existed
    # won't have this key yet. Derive it once from the manuscript so far.
    if "next_chapter_number" not in state:
        state["next_chapter_number"] = highest_chapter_number(state["manuscript"]) + 1

    chapter_number = state["next_chapter_number"]
...
This chapter is Chapter {chapter_number}. Begin the chapter with a heading
in exactly this form: "Chapter {chapter_number}" optionally followed by a
title (e.g. "Chapter {chapter_number} — Title"). Do not use any other
chapter number, and do not restart the numbering — {chapter_number} is the
correct, verified next chapter number for this manuscript regardless of
what number the story's internal narration might suggest.

Respond with only the chapter text, starting with that exact chapter heading."""
...
Keep the existing chapter heading exactly as it is — do not change the
chapter number.
...
def write_one_chapter(pid: str, state: dict) -> dict:
    ...
    if "next_chapter_number" not in state:
        state["next_chapter_number"] = highest_chapter_number(state["manuscript"]) + 1
    story_chapter_number = state["next_chapter_number"]
    ...
    state["next_chapter_number"] = story_chapter_number + 1

    new_word_count = word_count(state["manuscript"])
    save_project(pid, state)

    chap_num = state["chapters_written"]  # app's own file index, separate from the in-story chapter number
    (project_path(pid) / f"chapter_{chap_num:03d}.txt").write_text(chapter, encoding="utf-8")

    subject = f"'{state['title']}' — Chapter {story_chapter_number} ({new_word_count}/{state['target_words']} words)"
    send_email(state["email"], subject, chapter)
    print(f"[{pid}] wrote story chapter {story_chapter_number} (file #{chap_num}), {new_word_count} total words")
...
def create_project():
    ...
    state = {
        ...
        "running_summary": "",
        "chapters_written": 0,
        "next_chapter_number": highest_chapter_number(manuscript_text) + 1,
        "status": "active",
    }
    save_project(pid, state)
