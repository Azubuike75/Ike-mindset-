...
MODEL = "gemini-2.5-flash"  # free-tier model, generous daily quota
...
<ul>{% for p in projects %}<li><a href="/projects/{{p}}">{{p}}</a></li>{% endfor %}</ul>
</body></html>
"""
...
Chapter {{c.num}} — {% if c.published %}<span style="color:green">published</span>{% else %}<span style="color:#b35a00">draft, not published</span>
<button onclick="location.href='/projects/{{pid}}/publish/{{c.num}}'" style="padding:4px 10px;font-size:0.85em">Publish this one</button>
{% endif %}
</li>
{% endfor %}
</ul>
